...
knitr::kable(fit_pvals[1:10, ], row.names = TRUE, format = "markdown",
...
```
Based on table 2, we see that the 10 genes with the lowest p-values are completely different. Now, there are `r length(which(mhc$adj.P.Val < 0.05))` genes that have a p-value of less than 0.05, which is `r length(which(mhc$adj.P.Val < 0.05)) / nrow(mhc) * 100`%. 
...
`r length(which(mhc$adj.P.Val < 0.05))` genes passed correction.
